#ifndef ZMUONANALYZER 
#define ZMUONANALYZER 
...
//new includes needed for code specific to trigger that comes from Sam Harper
...
namespace{
  ...
  bool checkFilters(const float eta,const float phi,const std::vector<pat::TriggerObjectStandAlone>& trigObjs,const std::vector<std::string>& filterNames, std::vector<string> & temp_trig_vec, const float maxDeltaR=0.1)
  {
...
//	  std::cout <<" object "<<eta<<" "<<phi<<" passes "<<filterName<<std::endl; //commented out for speed
	  passAnyFilter=true;
	  temp_trig_vec.push_back(filterName);
	}
  ...
  bool checkFilters(const std::vector<pat::TriggerObjectStandAlone>& trigObjs,const std::vector<std::string>& filterNames)
  {
	...
	//the list of passed filters for this object
	if(std::find(objFilters.begin(),objFilters.end(),filterName)!=objFilters.end()){
//	  std::cout <<" object "<<trigObj.eta()<<" "<<trigObj.phi()<<" passes "<<filterName<<std::endl; //commented out for speed
	  passAnyFilter=true;
	}
      }//end loop over matched trigger objects
    }//end loop over filter lables
...
public:
	...
	//From Sam, needed for trigger specific code
	void beginRun(const edm::Run& run,const edm::EventSetup& setup) override; //14 July 2022 Fix later!!
...
//   edm::EDGetTokenT<std::vector<std::string>> muonMatch_;
   ...
   //variables from code from Sam Harper that deals with the trigger
...
//   int triggerYear;
   ...
   std::vector<std::vector<std::string>> lepton1_trigger_filters;  //FLAG 5 AUGUST 2022
  ...
  std::vector<std::vector<double>> dimuon1vtx_chi2, dimuon2vtx_chi2; //Bjorn
  std::vector<std::vector<int>> dimuon1vtx_ndof, dimuon2vtx_ndof; //Bjorn
...
ZmuonAnalyzer::ZmuonAnalyzer(const edm::ParameterSet& iConfig):
  hltConfig_(iConfig,consumesCollector(),*this) //fix thanks to Sam
...
//   hltConfig_ = (iConfig,consumesCollector(),*this); //Doesn't work for reasons Sam describes in our mattermost conversation

 //  hltConfig_ = HLTPrescaleProvider::HLTPrescaleProvider(iConfig,consumesCollector(),*this)
 //  trigResultsToken_= consumes<edm::TriggerResults>(triggerBits_);
 
  trigResultsTag_ = (iConfig.getParameter<edm::InputTag>("bits"));
  triggerBits_       = consumes<edm::TriggerResults>(trigResultsTag_);
  trigObjsToken_ = consumes<std::vector<pat::TriggerObjectStandAlone> >(iConfig.getParameter<edm::InputTag>("objects"));
...
//   tree->Branch("process_ID", &process_ID);
   ...
   tree->Branch("lepton1_trigger_filters", &lepton1_trigger_filters);
   ...
   tree->Branch("dimuon1vtx_chi2", &dimuon1vtx_chi2);
   tree->Branch("dimuon2vtx_chi2", &dimuon2vtx_chi2);
   
   tree->Branch("dimuon1vtx_ndof", &dimuon1vtx_ndof);
   tree->Branch("dimuon2vtx_ndof", &dimuon2vtx_ndof);
...
// *******
   ...
   //Code from Sam
   auto trigResultsHandle = iEvent.getHandle(triggerBits_);
...
//Recall that we get MC from the cfg setting, see:  isMC = iConfig.getParameter<bool>("isMC"); line above
   ...
   dimuon1vtx_chi2.clear(); dimuon2vtx_chi2.clear();
   dimuon1vtx_ndof.clear(); dimuon2vtx_ndof.clear();
   sixmuonvtx.clear();
...
//                 
            ...
            //    TransientVertex vtx2; vtx2 = kalman_fitter.vertex(transient_tracks_muon2);
             //   TransientVertex vtx3; vtx3 = kalman_fitter.vertex(transient_tracks_muon3);
              //  TransientVertex vtx4; vtx4 = kalman_fitter.vertex(transient_tracks_muon4);
                ...
                std::vector<double> dimuon1vtx_chi2_temp, dimuon2vtx_chi2_temp;
                dimuon1vtx_chi2_temp.clear(); dimuon2vtx_chi2_temp.clear();
                
                std::vector<int> dimuon1vtx_ndof_temp, dimuon2vtx_ndof_temp;
                dimuon1vtx_ndof_temp.clear(); dimuon2vtx_ndof_temp.clear();
                ...
                if (match_12_34_56) {
                 ...
                 //   std::cout << TMath::Prob(vtx12.totalChiSquared(), int(vtx12.degreesOfFreedom())) << std::endl;
                    ...
                    dimuon1vtx_chi2_temp.push_back(vtx12.totalChiSquared());
                    dimuon1vtx_ndof_temp.push_back(int(vtx12.degreesOfFreedom()));
                ...
                //    std::cout << "DEBUG 5C" << std::endl;
                 //   std::cout << vtx12.position().y() << std::endl; 
                    ...
                    dimuon1vtx_chi2_temp.push_back(-1000.);
                    dimuon1vtx_ndof_temp.push_back(-1000);
                  ...
                  if (vtx34.isValid()) {
                    ...
                    dimuon2vtx_chi2_temp.push_back(vtx34.totalChiSquared());
                    dimuon2vtx_ndof_temp.push_back(int(vtx34.degreesOfFreedom()));
                    ...
                    dimuon2vtx_chi2_temp.push_back(-1000.);
                    dimuon2vtx_ndof_temp.push_back(-1000);
                ...
                dimuon1vtx_chi2_temp.push_back(-10000.);
                dimuon1vtx_ndof_temp.push_back(-10000);
                dimuon2vtx_temp.push_back(-10000);
                dimuon2vtx_chi2_temp.push_back(-10000.);
                dimuon2vtx_ndof_temp.push_back(-10000);
                ...
                if (match_13_24_56) {
                  if (vtx13.isValid()) {
                    ...
                    dimuon1vtx_chi2_temp.push_back(vtx13.totalChiSquared());
                    dimuon1vtx_ndof_temp.push_back(int(vtx13.degreesOfFreedom()));
                  ...
                  if (vtx24.isValid()) {
                    ...
                    dimuon2vtx_chi2_temp.push_back(vtx24.totalChiSquared());
                    dimuon2vtx_ndof_temp.push_back(int(vtx24.degreesOfFreedom()));
                ...
                if (match_14_23_56) {
                  if (vtx14.isValid()) {
                    ...
                    dimuon1vtx_chi2_temp.push_back(vtx14.totalChiSquared());
                    dimuon1vtx_ndof_temp.push_back(int(vtx14.degreesOfFreedom()));
                  ...
                  if (vtx23.isValid()) {
                    ...
                    dimuon2vtx_chi2_temp.push_back(vtx23.totalChiSquared());
                    dimuon2vtx_ndof_temp.push_back(int(vtx23.degreesOfFreedom()));
     ...
     //               std::cout << "DEBUG 9" << std::endl;
                ...
                dimuon1vtx_chi2.push_back(dimuon1vtx_chi2_temp);
                dimuon1vtx_ndof.push_back(dimuon1vtx_ndof_temp);
                dimuon2vtx.push_back(dimuon2vtx_temp);
                dimuon2vtx_chi2.push_back(dimuon2vtx_chi2_temp);
                dimuon2vtx_ndof.push_back(dimuon2vtx_ndof_temp);
...
//    std::cout << "eventHasZUpsiNTo4Mu initially is: " << eventHasZUpsiNTo4Mu << std::endl;
    ...
    bool eventHasUpsi1ToMuMu = false;
//    std::cout << "eventHasUpsi1ToMuMu initially is: " << eventHasUpsi1ToMuMu << std::endl;
    
    bool eventHasUpsi2ToMuMu = false;
//    std::cout << "eventHasUpsi2ToMuMu initially is: " << eventHasUpsi2ToMuMu << std::endl;
